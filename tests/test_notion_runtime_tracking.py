...
- Live path → creates cockpit + all runtime databases + real ids in manifest
...
# ---------------------------------------------------------------------------
# Test: live path creates cockpit + runtime databases + real ids
# ---------------------------------------------------------------------------
...
def _build_live_responses(
    *,
    ...
    # POST /databases for each runtime database
...
def test_live_path_creates_cockpit_and_mapped_databases(tmp_path: Path) -> None:
    ...
    assert result.get("databases_created") == len(DB_NAMES)
...
def test_second_apply_is_idempotent(tmp_path: Path) -> None:
    ...
    # cockpit children (all runtime DBs present) + record_count * (query existing + patch)
    cockpit_block = {
        "id": COCKPIT_PAGE_ID,
        "type": "child_page",
    ...
    assert result.get("databases_reused") == len(DB_NAMES)
