---
name: run-logger
description: Record non-trivial OS execution into run logs and closeout surfaces. Use when the user asks for Agentic OS `run-logger` work or when the routed Agentic OS skill registry selects `run-logger`.
---
...
Harness-visible adapter for the Agentic OS `run-logger` skill.
...
3. Read `harness/rules/os-authoring-rules.md` before editing Agentic OS
   ...
   templates.
4. Read `harness/skills/run-logger/SKILL.md` after routing.
