...
description: Append events, inspect the ledger, test chain rules, and process follow-up work. Use when the user asks for Agentic OS `event-graph-operator` work or when the routed Agentic OS skill registry selects `event-graph-operator`.
---
...
Harness-visible adapter for the Agentic OS `event-graph-operator` skill.
...
3. Read `harness/rules/os-authoring-rules.md` before editing Agentic OS
   ...
   templates.
4. Read `harness/skills/event-graph-operator/SKILL.md` after routing.
