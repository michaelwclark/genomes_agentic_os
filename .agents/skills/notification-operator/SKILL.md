...
description: Send one governed local macOS notification for an operator-actionable Agentic OS condition without creating alert noise. Use when the user asks for Agentic OS `notification-operator` work or when the routed Agentic OS skill registry selects `notification-operator`.
---
...
Harness-visible adapter for the Agentic OS `notification-operator` skill.
...
3. Read `harness/rules/os-authoring-rules.md` before editing Agentic OS
   ...
   templates.
4. Read `harness/skills/notification-operator/SKILL.md` after routing.
