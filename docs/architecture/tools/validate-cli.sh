...
# regenerates a local validation results receipt (gitignored).
#
# Status legend:
...
#
# Re-run any time:  bash docs/architecture/tools/validate-cli.sh
set -u
...
RESULTS_MD="${AGENTIC_VALIDATE_RESULTS:-$REPO/.validation/RESULTS.md}"
...
# Regenerate durable markdown results matrix.
{
  ...
  echo
  echo "Generated by \`docs/architecture/tools/validate-cli.sh\` against a scratch root."
...
EXAMPLES_MD="${AGENTIC_VALIDATE_EXAMPLES:-$REPO/.validation/command-output-examples.md}"
{
  echo "# Real Command Output Examples"
  echo
  echo "Captured by \`docs/architecture/tools/validate-cli.sh\` against a scratch root."
