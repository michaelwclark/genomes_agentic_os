...
if [ "$mode" = --recreate ]; then
  ...
  then
    if jq -e '(.roleHealth | type)=="array"' "$status_temp" >/dev/null; then
      ...
      echo "control-plane status has no roleHealth array before recreate; relying on container replacement until post-recreate verification" >&2
    fi
  fi
