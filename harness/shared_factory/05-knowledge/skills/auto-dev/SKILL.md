...
When the operator explicitly asks Auto Dev to take a ticket "all the way to
PR", that is the delivery target for the run. The runner must continue through
commit, push, PR creation, and the tracker `pr_open` transition. A configured
human review checkpoint is not allowed to strand that delivery target: run the
configured reviewer transport, record a sanitized unavailable-review receipt
when the transport cannot authenticate or complete, and continue to PR. Actual
review findings, failed validation, CI failures, and product decisions remain
blocking. Do not merge unless the project merge policy and the operator request
both authorize it.
...
- Resolve every PR target family through
  `harness/shared_factory/05-knowledge/gitflow-topology/CONTRACT.md` and
  `resolve_gitflow_targets.py`. Project `target_policy` is data input; Auto-Dev
  must not interpret or copy GitFlow rules inline.
...
- `awaiting_human_review` is paused and non-error for ordinary runs. For an
  explicit `all the way to PR` delivery target, it is only a temporary
  compatibility state: execute the configured review transport and use the
  unavailable-review receipt path when needed so the run can reach `pr_open`.
  Release the local claim only when the run truly pauses or terminates; do not
  write a review-transport limitation to the tracker as `blocked`.
...
Keep the claim label while implementation is active because the LOS queue
dispatcher uses it as a cross-process duplicate-dispatch guard. Once the PR is
successfully opened and the tracker is moved to its configured `pr_open` state,
remove only the Auto Dev claim label and record the readback. Preserve all
other Jira labels.
...
It supplies `reviewer-prompt.md` as the explicit Claude print prompt and writes only stdout to
...
5. Plan and implement through `quiet-workon-orchestrate`, using the resolved
   project config. Implementation agents load DEV_STANDARDS
   before writing code: resolve the ordered folder list from
   `dev_factory.dev_standards.paths` (falling back to
   ...
   and enumerate every `*.md` except `README.md` in
   each (composable contract; new markdowns apply automatically), so the
   code is written to the same gates CI, Copilot, and the finalize review
   battery will score it against.
...
## Write-Side DEV_STANDARDS

The PR checks, Copilot review, and the `auto-dev-finalize` review battery all
score against one canonical composable standards plane: every `*.md` except
`README.md` in
`harness/shared_factory/05-knowledge/dev_standards/`, the routed domain
folder, and the routed project's `config/dev_standards/*.md` (for LOS,
starting with `LOS_CORE.md`; dropping a new markdown in a configured folder extends every
mission's next run). Write to
those gates during implementation so the finalize stage finds nothing:
reuse-before-reimplement, rule-of-three DRY, bounded complexity with explicit
None handling, batched ORM access with deliberate select/prefetch, tenant
scoping, branch-correct migration graphs, regression tests that reproduce the
incident data shape, and what/why/how comments at non-obvious seams. A gate
finding at finalize time is a write-side defect, not review noise.
...
Before creating a PR, save the F2 resolver verdict under
`artifacts/auto-dev/pr/topology.json`. Create the primary target and invoke
GitFlow PR Create for every additional target returned by `missing_targets`.
When the verdict contains multiple targets, hand the family-wide endgame to
`auto-dev-finalize`, which owns cross-PR watch, Copilot rounds, per-branch
migration audits, the deep quality gate, and the F2 family-completeness gate.
